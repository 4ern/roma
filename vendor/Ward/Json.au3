; ============================================================================================================================
; File		: Json.au3 (2015.01.08)
; Purpose	: A Non-Strict JavaScript Object Notation (JSON) Parser UDF
; Author	: Ward
; Dependency: BinaryCall.au3
; Website	: http://www.json.org/index.html
;
; Source	: jsmn.c
; Author	: zserge
; Website	: http://zserge.com/jsmn.html
;
; Source	: _vendor_ward_Jsonstring_encode.c, _vendor_ward_Jsonstring_decode.c
; Author	: Ward
; ============================================================================================================================

; ============================================================================================================================
; Public Functions:
;   _vendor_ward_JsonStringEncode($String, $Option = 0)
;   _vendor_ward_JsonStringDecode($String)
;   _vendor_ward_JsonIsObject(ByRef $Object)
;   _vendor_ward_JsonIsNull(ByRef $Null)
;   _vendor_ward_JsonEncode($Data, $Option = 0, $Indent = Default, $ArraySep = Default, $ObjectSep = Default, $ColonSep = Default)
;   _vendor_ward_JsonDecode($Json, $InitTokenCount = 1000)
;   _vendor_ward_JsonObjCreate()
;   _vendor_ward_JsonObjPut(ByRef $Object, $Key, $Value)
;   _vendor_ward_JsonObjGet(ByRef $Object, $Key)
;   _vendor_ward_JsonObjDelete(ByRef $Object, $Key)
;   _vendor_ward_JsonObjExists(ByRef $Object, $Key)
;   _vendor_ward_JsonObjGetCount(ByRef $Object)
;   _vendor_ward_JsonObjGetKeys(ByRef $Object)
;   _vendor_ward_JsonObjClear(ByRef $Object)
;   _vendor_ward_JsonPut(ByRef $Var, $Notation, $Data, $CheckExists = False)
;   _vendor_ward_JsonGet(ByRef $Var, $Notation)
; ============================================================================================================================

#Include-once
#Include "BinaryCall.au3"

; The following constants can be combined to form options for _vendor_ward_JsonEncode()
Global Const $_vendor_ward_JsonUNESCAPED_UNICODE	= 1		; Encode multibyte Unicode characters literally
Global Const $_vendor_ward_JsonUNESCAPED_SLASHES	= 2		; Don't escape /
Global Const $_vendor_ward_JsonHEX_TAG				= 4		; All < and > are converted to \u003C and \u003E
Global Const $_vendor_ward_JsonHEX_AMP				= 8		; All &s are converted to \u0026
Global Const $_vendor_ward_JsonHEX_APOS				= 16	; All ' are converted to \u0027
Global Const $_vendor_ward_JsonHEX_QUOT				= 32	; All " are converted to \u0022
Global Const $_vendor_ward_JsonUNESCAPED_ASCII		= 64	; Don't escape ascii charcters between chr(1) ~ chr(0x1f)
Global Const $_vendor_ward_JsonPRETTY_PRINT			= 128	; Use whitespace in returned data to format it
Global Const $_vendor_ward_JsonSTRICT_PRINT			= 256	; Make sure returned JSON string is RFC4627 compliant
Global Const $_vendor_ward_JsonUNQUOTED_STRING		= 512	; Output unquoted string if possible (conflicting with $_vendor_ward_JsonSTRICT_PRINT)

; Error value returnd by _vendor_ward_JsonDecode()
Global Const $JSMN_ERROR_NOMEM	= -1		; Not enough tokens were provided
Global Const $JSMN_ERROR_INVAL	= -2		; Invalid character inside JSON string
Global Const $JSMN_ERROR_PART	= -3		; The string is not a full JSON packet, more bytes expected

Func __Jsmn_RuntimeLoader($ProcName = "")
	Static $SymbolList
	If Not IsDllStruct($SymbolList) Then
		Local $Code
		If @AutoItX64 Then
			$Code = ''
		Else
			$Code = ''
		EndIf

		Local $Symbol[] = ["jsmn_parse","jsmn_init","_vendor_ward_Jsonstring_decode","_vendor_ward_Jsonstring_encode"]
		Local $CodeBase = _vendor_ward_BinaryCall_Create($Code)
		If @Error Then Exit MsgBox(16, "Json", "Startup Failure!")

		$SymbolList = _vendor_ward_BinaryCall_SymbolList($CodeBase, $Symbol)
		If @Error Then Exit MsgBox(16, "Json", "Startup Failure!")
	EndIf
	If $ProcName Then Return DllStructGetData($SymbolList, $ProcName)
EndFunc

Func _vendor_ward_JsonStringEncode($String, $Option = 0)
	Static $_vendor_ward_JsonStringEncode = __Jsmn_RuntimeLoader("_vendor_ward_Jsonstring_encode")
	Local $Length = StringLen($String) * 6 + 1
	Local $Buffer = DllStructCreate("wchar[" & $Length & "]")
	Local $Ret = DllCallAddress("int:cdecl", $_vendor_ward_JsonStringEncode, "wstr", $String, "ptr", DllStructGetPtr($Buffer), "uint", $Length, "int", $Option)
	Return SetError($Ret[0], 0, DllStructGetData($Buffer, 1))
EndFunc

Func _vendor_ward_JsonStringDecode($String)
	Static $_vendor_ward_JsonStringDecode = __Jsmn_RuntimeLoader("_vendor_ward_Jsonstring_decode")
	Local $Length = StringLen($String) + 1
	Local $Buffer = DllStructCreate("wchar[" & $Length & "]")
	Local $Ret = DllCallAddress("int:cdecl", $_vendor_ward_JsonStringDecode, "wstr", $String, "ptr", DllStructGetPtr($Buffer), "uint", $Length)
	Return SetError($Ret[0], 0, DllStructGetData($Buffer, 1))
EndFunc

Func _vendor_ward_JsonDecode($Json, $InitTokenCount = 1000)
	Static $Jsmn_Init = __Jsmn_RuntimeLoader("jsmn_init"), $Jsmn_Parse = __Jsmn_RuntimeLoader("jsmn_parse")
	If $Json = "" Then $Json = '""'
	Local $TokenList, $Ret
	Local $Parser = DllStructCreate("uint pos;int toknext;int toksuper")
	Do
		DllCallAddress("none:cdecl", $Jsmn_Init, "ptr", DllStructGetPtr($Parser))
		$TokenList = DllStructCreate("byte[" & ($InitTokenCount * 20) & "]")
		$Ret = DllCallAddress("int:cdecl", $Jsmn_Parse, "ptr", DllStructGetPtr($Parser), "wstr", $Json, "ptr", DllStructGetPtr($TokenList), "uint", $InitTokenCount)
		$InitTokenCount *= 2
	Until $Ret[0] <> $JSMN_ERROR_NOMEM

	Local $Next = 0
	Return SetError($Ret[0], 0, __vendor_ward_JsonToken($Json, DllStructGetPtr($TokenList), $Next))
EndFunc

Func __vendor_ward_JsonToken(ByRef $Json, $Ptr, ByRef $Next)
	If $Next = -1 Then Return Null

	Local $Token = DllStructCreate("int;int;int;int", $Ptr + ($Next * 20))
	Local $Type = DllStructGetData($Token, 1)
	Local $Start = DllStructGetData($Token, 2)
	Local $End = DllStructGetData($Token, 3)
	Local $Size = DllStructGetData($Token, 4)
	$Next += 1

	If $Type = 0 And $Start = 0 And $End = 0 And $Size = 0 Then ; Null Item
		$Next = -1
		Return Null
	EndIf

	Switch $Type
		Case 0 ; _vendor_ward_JsonPRIMITIVE
			Local $Primitive = StringMid($Json, $Start + 1, $End - $Start)
			Switch $Primitive
				Case "true"
					Return True
				Case "false"
					Return False
				Case "null"
					Return Null
				Case Else
					If StringRegExp($Primitive, "^[+\-0-9]") Then
						Return Number($Primitive)
					Else
						Return _vendor_ward_JsonStringDecode($Primitive)
					EndIf
			EndSwitch

		Case 1 ; _vendor_ward_JsonOBJECT
			Local $Object = _vendor_ward_JsonObjCreate()
			For $i = 0 To $Size - 1 Step 2
				Local $Key = __vendor_ward_JsonToken($Json, $Ptr, $Next)
				Local $Value = __vendor_ward_JsonToken($Json, $Ptr, $Next)
				If Not IsString($Key) Then $Key = _vendor_ward_JsonEncode($Key)

				If $Object.Exists($Key) Then $Object.Remove($Key)
				$Object.Add($Key, $Value)
			Next
			Return $Object

		Case 2 ; _vendor_ward_JsonARRAY
			Local $Array[$Size]
			For $i = 0 To $Size - 1
				$Array[$i] = __vendor_ward_JsonToken($Json, $Ptr, $Next)
			Next
			Return $Array

		Case 3 ; _vendor_ward_JsonSTRING
			Return _vendor_ward_JsonStringDecode(StringMid($Json, $Start + 1, $End - $Start))
	EndSwitch
EndFunc

Func _vendor_ward_JsonIsObject(ByRef $Object)
	Return (IsObj($Object) And ObjName($Object) = "Dictionary")
EndFunc

Func _vendor_ward_JsonIsNull(ByRef $Null)
	Return IsKeyword($Null) Or (Not IsObj($Null) And VarGetType($Null) = "Object")
EndFunc

Func _vendor_ward_JsonEncode_Compact($Data, $Option = 0)
	Select
		Case IsString($Data)
			Return '"' & _vendor_ward_JsonStringEncode($Data, $Option) & '"'

		Case IsNumber($Data)
			Return $Data

		Case IsArray($Data) And UBound($Data, 0) = 1
			Local $Json = "["
			For $i = 0 To UBound($Data) - 1
				$Json &= _vendor_ward_JsonEncode_Compact($Data[$i], $Option) & ","
			Next
			If StringRight($Json, 1) = "," Then $Json = StringTrimRight($Json, 1)
			Return $Json & "]"

		Case _vendor_ward_JsonIsObject($Data)
			Local $Json = "{"
			Local $Keys = $Data.Keys()
			For $i = 0 To UBound($Keys) - 1
				$Json &= '"' & _vendor_ward_JsonStringEncode($Keys[$i], $Option) & '":' & _vendor_ward_JsonEncode_Compact($Data.Item($Keys[$i]), $Option) & ","
			Next
			If StringRight($Json, 1) = "," Then $Json = StringTrimRight($Json, 1)
			Return $Json & "}"

		Case IsBool($Data)
			Return StringLower($Data)

		Case IsPtr($Data)
			Return Number($Data)

		Case IsBinary($Data)
			Return '"' & _vendor_ward_JsonStringEncode(BinaryToString($Data, 4), $Option) & '"'

		Case Else ; Keyword, DllStruct, Object
			Return "null"
	EndSelect
EndFunc

Func _vendor_ward_JsonEncode_Pretty($Data, $Option, $Indent, $ArraySep, $ObjectSep, $ColonSep, $ArrayCRLF = Default, $ObjectCRLF = Default, $NextIdent = "")
	Local $ThisIdent = $NextIdent, $Json = ""
	Select
		Case IsString($Data)
			Local $String = _vendor_ward_JsonStringEncode($Data, $Option)
			If BitAND($Option, $_vendor_ward_JsonUNQUOTED_STRING) And Not BitAND($Option, $_vendor_ward_JsonSTRICT_PRINT) And Not StringRegExp($String, "[\s,:]") And Not StringRegExp($String, "^[+\-0-9]") Then
				Return $String
			Else
				Return '"' & $String & '"'
			EndIf

		Case IsArray($Data) And UBound($Data, 0) = 1
			If UBound($Data) = 0 Then Return "[]"
			If IsKeyword($ArrayCRLF) Then
				$ArrayCRLF = ""
				Local $Match = StringRegExp($ArraySep, "[\r\n]+$", 3)
				If IsArray($Match) Then $ArrayCRLF = $Match[0]
			EndIf

			If $ArrayCRLF Then $NextIdent &= $Indent
			Local $Length = UBound($Data) - 1
			For $i = 0 To $Length
				If $ArrayCRLF Then $Json &= $NextIdent
				$Json &= _vendor_ward_JsonEncode_Pretty($Data[$i], $Option, $Indent, $ArraySep, $ObjectSep, $ColonSep, $ArrayCRLF, $ObjectCRLF, $NextIdent)
				If $i < $Length Then $Json &= $ArraySep
			Next

			If $ArrayCRLF Then Return "[" & $ArrayCRLF & $Json & $ArrayCRLF & $ThisIdent & "]"
			Return "[" & $Json & "]"

		Case _vendor_ward_JsonIsObject($Data)
			If $Data.Count = 0 Then Return "{}"
			If IsKeyword($ObjectCRLF) Then
				$ObjectCRLF = ""
				Local $Match = StringRegExp($ObjectSep, "[\r\n]+$", 3)
				If IsArray($Match) Then $ObjectCRLF = $Match[0]
			EndIf

			If $ObjectCRLF Then $NextIdent &= $Indent
			Local $Keys = $Data.Keys()
			Local $Length = UBound($Keys) - 1
			For $i = 0 To $Length
				If $ObjectCRLF Then $Json &= $NextIdent
				$Json &= _vendor_ward_JsonEncode_Pretty(String($Keys[$i]), $Option, $Indent, $ArraySep, $ObjectSep, $ColonSep) & $ColonSep _
						& _vendor_ward_JsonEncode_Pretty($Data.Item($Keys[$i]), $Option, $Indent, $ArraySep, $ObjectSep, $ColonSep, $ArrayCRLF, $ObjectCRLF, $NextIdent)
				If $i < $Length Then $Json &= $ObjectSep
			Next

			If $ObjectCRLF Then Return "{" & $ObjectCRLF & $Json & $ObjectCRLF & $ThisIdent & "}"
			Return "{" & $Json & "}"

		Case Else
			Return _vendor_ward_JsonEncode_Compact($Data, $Option)

	EndSelect
EndFunc

Func _vendor_ward_JsonEncode($Data, $Option = 0, $Indent = Default, $ArraySep = Default, $ObjectSep = Default, $ColonSep = Default)
	If BitAND($Option, $_vendor_ward_JsonPRETTY_PRINT) Then
		Local $Strict = BitAND($Option, $_vendor_ward_JsonSTRICT_PRINT)

		If IsKeyword($Indent) Then
			$Indent = @Tab
		Else
			$Indent = _vendor_ward_JsonStringDecode($Indent)
			If StringRegExp($Indent, "[^\t ]") Then $Indent = @Tab
		EndIf

		If IsKeyword($ArraySep) Then
			$ArraySep = "," & @CRLF
		Else
			$ArraySep = _vendor_ward_JsonStringDecode($ArraySep)
			If $ArraySep = "" Or StringRegExp($ArraySep, "[^\s,]|,.*,") Or ($Strict And Not StringRegExp($ArraySep, ",")) Then $ArraySep = "," & @CRLF
		EndIf

		If IsKeyword($ObjectSep) Then
			$ObjectSep = "," & @CRLF
		Else
			$ObjectSep = _vendor_ward_JsonStringDecode($ObjectSep)
			If $ObjectSep = "" Or StringRegExp($ObjectSep, "[^\s,]|,.*,") Or ($Strict And Not StringRegExp($ObjectSep, ",")) Then $ObjectSep = "," & @CRLF
		EndIf

		If IsKeyword($ColonSep) Then
			$ColonSep = ": "
		Else
			$ColonSep = _vendor_ward_JsonStringDecode($ColonSep)
			If $ColonSep = "" Or StringRegExp($ColonSep, "[^\s,:]|[,:].*[,:]") Or ($Strict And (StringRegExp($ColonSep, ",") Or Not StringRegExp($ColonSep, ":"))) Then $ColonSep = ": "
		EndIf

		Return _vendor_ward_JsonEncode_Pretty($Data, $Option, $Indent, $ArraySep, $ObjectSep, $ColonSep)

	ElseIf BitAND($Option, $_vendor_ward_JsonUNQUOTED_STRING) Then
		Return _vendor_ward_JsonEncode_Pretty($Data, $Option, "", ",", ",", ":")
	Else
		Return _vendor_ward_JsonEncode_Compact($Data, $Option)
	EndIf
EndFunc

Func _vendor_ward_JsonObjCreate()
	Local $Object = ObjCreate('Scripting.Dictionary')
	$Object.CompareMode = 0
	Return $Object
EndFunc

Func _vendor_ward_JsonObjPut(ByRef $Object, $Key, $Value)
	$Key = String($Key)
	If $Object.Exists($Key) Then $Object.Remove($Key)
	$Object.Add($Key, $Value)
EndFunc

Func _vendor_ward_JsonObjGet(ByRef $Object, $Key)
	$Key = String($Key)
	If $Object.Exists($Key) Then Return $Object.Item($Key)
	Return SetError(1, 0, '')
EndFunc

Func _vendor_ward_JsonObjDelete(ByRef $Object, $Key)
	$Key = String($Key)
	If $Object.Exists($Key) Then $Object.Remove($Key)
EndFunc

Func _vendor_ward_JsonObjExists(ByRef $Object, $Key)
	Return $Object.Exists(String($Key))
EndFunc

Func _vendor_ward_JsonObjGetCount(ByRef $Object)
	Return $Object.Count
EndFunc

Func _vendor_ward_JsonObjGetKeys(ByRef $Object)
	Return $Object.Keys()
EndFunc

Func _vendor_ward_JsonObjClear(ByRef $Object)
	Return $Object.RemoveAll()
EndFunc

; Both dot notation and square bracket notation can be supported
Func _vendor_ward_JsonPut(ByRef $Var, $Notation, $Data, $CheckExists = False)
	Local $Match = StringRegExp($Notation, "(^\[([^\]]+)\])|(^\.([^\.\[]+))", 3)
	If IsArray($Match) Then
		Local $Index
		If UBound($Match) = 4 Then
			$Index = String(_vendor_ward_JsonDecode($Match[3])) ; only string using dot notation
			$Notation = StringTrimLeft($Notation, StringLen($Match[2]))
		Else
			$Index = _vendor_ward_JsonDecode($Match[1])
			$Notation = StringTrimLeft($Notation, StringLen($Match[0]))
		EndIf

		If IsString($Index) Then
			If $CheckExists And (Not _vendor_ward_JsonIsObject($Var) Or Not _vendor_ward_JsonObjExists($Var, $Index)) Then
				Return SetError(1, 0, False) ; no specific object
			EndIf

			If Not _vendor_ward_JsonIsObject($Var) Then $Var = _vendor_ward_JsonObjCreate()
			If $Notation Then
				Local $Item = _vendor_ward_JsonObjGet($Var, $Index)
				Local $Ret = _vendor_ward_JsonPut($Item, $Notation, $Data, $CheckExists)
				Local $Error = @Error
				If Not $Error Then _vendor_ward_JsonObjPut($Var, $Index, $Item)
				Return SetError($Error, 0, $Ret)
			Else
				_vendor_ward_JsonObjPut($Var, $Index, $Data)
				Return True
			EndIf

		ElseIf IsInt($Index) Then
			If $Index < 0 Or ($CheckExists And (Not IsArray($Var) Or UBound($Var, 0) <> 1 Or $Index >= UBound($Var))) Then
				Return SetError(1, 0, False) ; no specific object
			EndIf

			If Not IsArray($Var) Or UBound($Var, 0) <> 1 Then
				Dim $Var[$Index + 1]
			ElseIf $Index >= UBound($Var) Then
				ReDim $Var[$Index + 1]
			EndIf

			If $Notation Then
				Local $Ret = _vendor_ward_JsonPut($Var[$Index], $Notation, $Data, $CheckExists)
				Return SetError(@Error, 0, $Ret)
			Else
				$Var[$Index] = $Data
				Return True
			EndIf

		EndIf
	EndIf
	Return SetError(2, 0, False) ; invalid notation
EndFunc

; Both dot notation and square bracket notation can be supported
Func _vendor_ward_JsonGet(ByRef $Var, $Notation)
	Local $Match = StringRegExp($Notation, "(^\[([^\]]+)\])|(^\.([^\.\[]+))", 3)
	If IsArray($Match) Then
		Local $Index
		If UBound($Match) = 4 Then
			$Index = String(_vendor_ward_JsonDecode($Match[3])) ; only string using dot notation
			$Notation = StringTrimLeft($Notation, StringLen($Match[2]))
		Else
			$Index = _vendor_ward_JsonDecode($Match[1])
			$Notation = StringTrimLeft($Notation, StringLen($Match[0]))
		EndIf

		Local $Item
		If IsString($Index) And _vendor_ward_JsonIsObject($Var) And _vendor_ward_JsonObjExists($Var, $Index) Then
			$Item = _vendor_ward_JsonObjGet($Var, $Index)
		ElseIf IsInt($Index) And IsArray($Var) And UBound($Var, 0) = 1 And $Index >= 0 And $Index < UBound($Var) Then
			$Item = $Var[$Index]
		Else
			Return SetError(1, 0, "") ; no specific object
		EndIf

		If Not $Notation Then Return $Item
		Local $Ret = _vendor_ward_JsonGet($Item, $Notation)
		Return SetError(@Error, 0, $Ret)
	EndIf
	Return SetError(2, 0, "") ; invalid notation
EndFunc
