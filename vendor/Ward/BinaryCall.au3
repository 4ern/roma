...
Global $__vendor_ward_BinaryCall_Kernel32dll = DllOpen('kernel32.dll')
Global $__vendor_ward_BinaryCall_Msvcrtdll = DllOpen('msvcrt.dll')
Global $__vendor_ward_BinaryCall_LastError = ""

Func _vendor_ward_BinaryCall_GetProcAddress($Module, $Proc)
	Local $Ret = DllCall($__vendor_ward_BinaryCall_Kernel32dll, 'ptr', 'GetProcAddress', 'ptr', $Module, 'str', $Proc)
	If @Error Or Not $Ret[0] Then Return SetError(1, @Error, 0)
	Return $Ret[0]
EndFunc

Func _vendor_ward_BinaryCall_LoadLibrary($Filename)
	Local $Ret = DllCall($__vendor_ward_BinaryCall_Kernel32dll, "handle", "LoadLibraryW", "wstr", $Filename)
	If @Error Then Return SetError(1, @Error, 0)
	Return $Ret[0]
EndFunc

Func _vendor_ward_BinaryCall_lstrlenA($Ptr)
	Local $Ret = DllCall($__vendor_ward_BinaryCall_Kernel32dll, "int", "lstrlenA", "ptr", $Ptr)
	If @Error Then Return SetError(1, @Error, 0)
	Return $Ret[0]
EndFunc

Func _vendor_ward_BinaryCall_Alloc($Code, $Padding = 0)
	Local $Length = BinaryLen($Code) + $Padding
	Local $Ret = DllCall($__vendor_ward_BinaryCall_Kernel32dll, "ptr", "VirtualAlloc", "ptr", 0, "ulong_ptr", $Length, "dword", 0x1000, "dword", 0x40)
	If @Error Or Not $Ret[0] Then Return SetError(1, @Error, 0)
...
Func _vendor_ward_BinaryCall_RegionSize($Ptr)
	Local $Buffer = DllStructCreate("ptr;ptr;dword;uint_ptr;dword;dword;dword")
	Local $Ret = DllCall($__vendor_ward_BinaryCall_Kernel32dll, "int", "VirtualQuery", "ptr", $Ptr, "ptr", DllStructGetPtr($Buffer), "uint_ptr", DllStructGetSize($Buffer))
	If @Error Or $Ret[0] = 0 Then Return SetError(1, @Error, 0)
	Return DllStructGetData($Buffer, 4)
EndFunc

Func _vendor_ward_BinaryCall_Free($Ptr)
	Local $Ret = DllCall($__vendor_ward_BinaryCall_Kernel32dll, "bool", "VirtualFree", "ptr", $Ptr, "ulong_ptr", 0, "dword", 0x8000)
	If @Error Or $Ret[0] = 0 Then
		$Ret = DllCall($__vendor_ward_BinaryCall_Kernel32dll, "bool", "GlobalFree", "ptr", $Ptr)
		If @Error Or $Ret[0] <> 0 Then Return SetError(1, @Error, False)
	EndIf
	Return True
EndFunc

Func _vendor_ward_BinaryCall_Release($CodeBase)
	Local $Ret = _vendor_ward_BinaryCall_Free($CodeBase)
	Return SetError(@Error, @Extended, $Ret)
EndFunc

Func _vendor_ward_BinaryCall_MemorySearch($Ptr, $Length, $Binary)
	Static $CodeBase
	If Not $CodeBase Then
		If @AutoItX64 Then
			$CodeBase = _vendor_ward_BinaryCall_Create('0x4883EC084D85C94889C8742C4C39CA72254C29CA488D141131C9EB0848FFC14C39C97414448A1408453A140874EE48FFC04839D076E231C05AC3', '', 0, True, False)
		Else
			$CodeBase = _vendor_ward_BinaryCall_Create('0x5589E58B4D14578B4508568B550C538B7D1085C9742139CA721B29CA8D341031D2EB054239CA740F8A1C17381C1074F34039F076EA31C05B5E5F5DC3', '', 0, True, False)
		EndIf
...
Func _vendor_ward_BinaryCall_Base64Decode($Src)
	Static $CodeBase
	If Not $CodeBase Then
		If @AutoItX64 Then
			$CodeBase = _vendor_ward_BinaryCall_Create('', '', 132, True, False)
		Else
			$CodeBase = _vendor_ward_BinaryCall_Create('', '', 132, True, False)
		EndIf
...
Func _vendor_ward_BinaryCall_LzmaDecompress($Src)
	...
	If $Ret[0] Then
		...
		DllCall($__vendor_ward_BinaryCall_Msvcrtdll, "none:cdecl", "free", "ptr", $Ret[0])
...
Func _vendor_ward_BinaryCall_Relocation($Base, $Reloc)
...
Func _vendor_ward_BinaryCall_ImportLibrary($Base, $Length)
	...
	Do
		Local $Ptr = _vendor_ward_BinaryCall_MemorySearch($Base, $Length, $JmpBin)
		If $Ptr = 0 Then ExitLoop

		Local $StringPtr = $Ptr + $JmpLen
		Local $StringLen = _vendor_ward_BinaryCall_lstrlenA($StringPtr)
		...
		If $DllName And $ProcName Then
			Local $Handle = _vendor_ward_BinaryCall_LoadLibrary($DllName)
			If Not $Handle Then
				$__vendor_ward_BinaryCall_LastError = "LoadLibrary fail on " & $DllName
				Return SetError(1, 0, False)
			EndIf

			Local $Proc = _vendor_ward_BinaryCall_GetProcAddress($Handle, $ProcName)
			If Not $Proc Then
				$__vendor_ward_BinaryCall_LastError = "GetProcAddress failed on " & $ProcName
...
Func _vendor_ward_BinaryCall_CodePrepare($Code)
	...
	Return _vendor_ward_BinaryCall_LzmaDecompress(_vendor_ward_BinaryCall_Base64Decode($Code))
EndFunc

Func _vendor_ward_BinaryCall_SymbolFind($CodeBase, $Identify, $Length = Default)
	$Identify = Binary($Identify)

	If IsKeyword($Length) Then
		$Length = _vendor_ward_BinaryCall_RegionSize($CodeBase)
	EndIf

	Local $Ptr = _vendor_ward_BinaryCall_MemorySearch($CodeBase, $Length, $Identify)
	If $Ptr = 0 Then Return SetError(1, 0, 0)

	Return $Ptr + BinaryLen($Identify)
EndFunc

Func _vendor_ward_BinaryCall_SymbolList($CodeBase, $Symbol)
	...
	For $i = 0 To UBound($Symbol) - 1
		$CodeBase = _vendor_ward_BinaryCall_SymbolFind($CodeBase, $Symbol[$i])
		DllStructSetData($SymbolList, $Symbol[$i], $CodeBase)
	Next
	Return $SymbolList
EndFunc

Func _vendor_ward_BinaryCall_Create($Code, $Reloc = '', $Padding = 0, $ReleaseOnExit = True, $LibraryImport = True)
	Local $BinaryCode = _vendor_ward_BinaryCall_CodePrepare($Code)
	...
	Local $CodeBase = _vendor_ward_BinaryCall_Alloc($BinaryCode, $Padding)
	If Not $CodeBase Then Return SetError(2, 0, 0)

	If $Reloc Then
		$Reloc = _vendor_ward_BinaryCall_CodePrepare($Reloc)
		If Not $Reloc Then Return SetError(3, 0, 0)
		_vendor_ward_BinaryCall_Relocation($CodeBase, $Reloc)
	EndIf

	If $LibraryImport Then
		If Not _vendor_ward_BinaryCall_ImportLibrary($CodeBase, $BinaryCodeLen) Then
			_vendor_ward_BinaryCall_Free($CodeBase)
			Return SetError(4, 0, 0)
		EndIf
	EndIf

	If $ReleaseOnExit Then
		_vendor_ward_BinaryCall_ReleaseOnExit($CodeBase)
	EndIf

	Return SetError(0, $TotalCodeLen, $CodeBase)
EndFunc

Func _vendor_ward_BinaryCall_CommandLineToArgv($CommandLine, ByRef $Argc, $IsUnicode = False)
	Static $SymbolList
	If Not IsDllStruct($SymbolList) Then
		...
		EndIf
		Local $CodeBase = _vendor_ward_BinaryCall_Create($Code)
		If @Error Then Return SetError(1, 0, 0)

		Local $Symbol[] = ["ToArgvW","ToArgvA"]
		$SymbolList = _vendor_ward_BinaryCall_SymbolList($CodeBase, $Symbol)
		If @Error Then Return SetError(1, 0, 0)
	EndIf
	...
	If Not @Error And $Ret[0] <> 0 Then
		_vendor_ward_BinaryCall_ReleaseOnExit($Ret[0])
		$Argc = $Ret[2]
		Return $Ret[0]
	Else
...
Func _vendor_ward_BinaryCall_StdioRedirect($Filename = "CON", $Flag = 1 + 2 + 4)
	Static $SymbolList
	If Not IsDllStruct($SymbolList) Then
		Local $Code, $Reloc
		If @AutoItX64 Then
		...
		Local $CodeBase = _vendor_ward_BinaryCall_Create($Code, $Reloc)
		If @Error Then Return SetError(1, 0, 0)
...
Func _vendor_ward_BinaryCall_StdinRedirect($Filename = "CON")
	Local $Ret = _vendor_ward_BinaryCall_StdioRedirect($Filename, 1)
	Return SetError(@Error, @Extended, $Ret)
EndFunc

Func _vendor_ward_BinaryCall_StdoutRedirect($Filename = "CON")
	Local $Ret = _vendor_ward_BinaryCall_StdioRedirect($Filename, 2)
	Return SetError(@Error, @Extended, $Ret)
EndFunc

Func _vendor_ward_BinaryCall_StderrRedirect($Filename = "CON")
	Local $Ret = _vendor_ward_BinaryCall_StdioRedirect($Filename, 4)
	Return SetError(@Error, @Extended, $Ret)
EndFunc

Func _vendor_ward_BinaryCall_ReleaseOnExit($Ptr)
	OnAutoItExitRegister('__vendor_ward_BinaryCall_DoRelease')
	__vendor_ward_BinaryCall_ReleaseOnExit_Handle($Ptr)
EndFunc

Func __vendor_ward_BinaryCall_DoRelease()
	__vendor_ward_BinaryCall_ReleaseOnExit_Handle()
EndFunc

Func __vendor_ward_BinaryCall_ReleaseOnExit_Handle($Ptr = Default)
	Static $PtrList

	If @NumParams = 0 Then
		If IsArray($PtrList) Then
			For $i = 1 To $PtrList[0]
				_vendor_ward_BinaryCall_Free($PtrList[$i])
			Next
		EndIf
	Else
